# AuralSong Specification

## Purpose
A **AuralSong** is the only runtime input the game needs.

- Ingestion converts source audio (and optionally MIDI) into an AuralSong.
- The desktop app + visualizers consume AuralSongs deterministically.

## Container
Two interchangeable container forms:

1) **Directory AuralSong** (for development)
- `MySong.auralsong/` as a folder.

2) **Zip AuralSong** (for distribution)
- `MySong.auralsong` is a zip file.
...
- The host treats any `*.auralsong` file as a zip AuralSong.
- The host treats any `*.auralsong/` directory as a directory AuralSong.
- Recommended UX: scan a configured songs folder on startup and index any AuralSongs found.
...
### Audio codec policy
- AuralSongs may include compressed audio (`mix.mp3` and/or `mix.ogg`) to reduce size.
...
AuralSongs may include timed lyrics for karaoke-style playback.
