...
Regression context: a Psalm 19 AuralSong imported via `aural_ingest import` did
...
SCHEMA_PATH = REPO_ROOT / "packages" / "auralsong" / "schemas" / "manifest.schema.json"
...
def _emit_init_manifest(*, title: str = "Test Song", artist: str = "Test Artist") -> dict:
    """Reproduce the manifest aural_ingest writes during the ``init_auralsong`` stage.
...
def test_init_manifest_passes_game_schema() -> None:
    ...
    should still discover the auralsong (Rust scan is permissive) and the
    schema should still pass so the strict validateAuralSong path doesn't
    reject it.
    """
    schema = _load_schema()
...
def test_required_fields_match_game_schema() -> None:
    ...
    assert not missing, (
        ...
        f"Update python/ingest/src/aural_ingest/cli.py init_auralsong stage."
    )
...
def test_empty_artist_and_zero_duration_still_valid() -> None:
    ...
    schema or partially-imported auralsongs become undiscoverable."""
    schema = _load_schema()
    jsonschema = _jsonschema_or_skip()
...
def test_init_manifest_matches_cli_source() -> None:
    """Drift guard: if cli.py's init_auralsong block adds/removes a top-level
...
def test_manifest_written_by_real_init_stage_passes_schema(tmp_path) -> None:
    """End-to-end: invoke the init_auralsong stage with a tiny synthetic wav
    ...
    # Synthesize a 0.5s silent wav so init_auralsong runs to completion. Skip if
    ...
    # Run only the init_auralsong stage. We reach into cli.py to avoid the heavy
