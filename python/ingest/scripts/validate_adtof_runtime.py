"""Validate the opt-in ADTOF runtime contract.

This script runs in the normal ingest environment. The TensorFlow/ADTOF
runtime remains outside the sidecar and is invoked by ``adtof_drums`` through
the configured external Python and official ADTOF checkout.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("wav_path", type=Path, help="Drum stem WAV to pass through the ADTOF runner.")
    ...
    if args.write_gate_evidence:
        ...
        output = _gate_evidence_output_path("adtof")
    ...
    report = adtof_drums.validate_runtime(wav_path, require_events=bool(args.require_events))
