"""Validate the opt-in QMUL high-resolution guitar runtime contract.

This script runs in the normal ingest environment. The QMUL-compatible
transcription runtime remains outside the sidecar and is invoked by
``qmul_hr_guitar`` through the configured external Python/repo/command.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("wav_path", type=Path, help="Guitar stem WAV to pass through the QMUL runner.")
    ...
    parser.add_argument(
        "--instrument",
        default="lead_guitar",
        help="Instrument label to stamp on returned notes.",
    )
    parser.add_argument(
        "--require-notes",
        action="store_true",
        help="Fail if the runner contract succeeds but returns zero guitar notes.",
    )
    ...
    if args.write_gate_evidence:
        ...
        if not args.require_notes:
            parser.error("--write-gate-evidence requires --require-notes")
        output = _gate_evidence_output_path("qmul_hr_guitar")
    ...
    from aural_ingest.algorithms import qmul_hr_guitar

    report = qmul_hr_guitar.validate_runtime(
        wav_path,
        instrument=str(args.instrument),
