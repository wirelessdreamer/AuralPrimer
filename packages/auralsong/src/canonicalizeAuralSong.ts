...
export type CanonicalizeAuralSongOptions = {
...
/**
 * Canonicalize JSON files in-place in a directory AuralSong.
 *
 * This is a deterministic normalization step: stable key ordering + float quantization.
 */
export async function canonicalizeAuralSongDirectory(auralSongDir: string, opts: CanonicalizeAuralSongOptions = {}): Promise<void> {
  const jsonOpts: CanonicalJsonOptions = { ...defaultJsonOpts(), ...(opts.json ?? {}) };

  for await (const rel of walkFiles(auralSongDir)) {
...
/**
 * Create a deterministic zip from a zip AuralSong by reserializing JSON entries canonically.
 */
export async function canonicalizeAuralSongZipBytes(zipBytes: Uint8Array, opts: CanonicalizeAuralSongOptions = {}): Promise<Uint8Array> {
  const jsonOpts: CanonicalJsonOptions = { ...defaultJsonOpts(), ...(opts.json ?? {}) };
...
/**
 * Read a zip AuralSong from disk, canonicalize it, and write it back.
 */
export async function canonicalizeAuralSongZipInPlace(zipAuralSongPath: string, opts: CanonicalizeAuralSongOptions = {}): Promise<void> {
  const raw = await fs.readFile(zipAuralSongPath);
  const out = await canonicalizeAuralSongZipBytes(new Uint8Array(raw), opts);
  await fs.writeFile(zipAuralSongPath, Buffer.from(out));
}
