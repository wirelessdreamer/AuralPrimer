...
import { validateAuralSong } from "./validateAuralSong";

export interface BuildAuralSongZipOptions {
  /**
   * When true (default), validate the input directory AuralSong before zipping.
   *
...
/**
 * Build a deterministic `.auralsong` zip (as bytes) from a directory AuralSong.
 */
export async function buildAuralSongZipFromDirectory(
  auralSongDir: string,
  opts: BuildAuralSongZipOptions = {}
): Promise<Uint8Array> {
  ...
  const st = await fs.stat(auralSongDir);
  if (!st.isDirectory()) {
    throw new Error(`buildAuralSongZipFromDirectory expects a directory: ${auralSongDir}`);
  }

  if (validate) {
    const res = await validateAuralSong(auralSongDir);
    if (!res.ok) {
      const msg = res.issues.map((i) => `${i.code}:${i.path}`).join(", ");
      throw new Error(`AuralSong validation failed (${res.containerKind}): ${msg}`);
    }
  }

  const relPaths: string[] = [];
  for await (const rel of walkFiles(auralSongDir)) relPaths.push(rel);
  relPaths.sort();

  const z: Zippable = {};
  for (const rel of relPaths) {
    const abs = path.join(auralSongDir, rel);
 ...
 */
export async function writeAuralSongZipFromDirectory(
  auralSongDir: string,
  outAuralSongZipPath: string,
  opts: BuildAuralSongZipOptions = {}
): Promise<void> {
  const bytes = await buildAuralSongZipFromDirectory(auralSongDir, opts);
  await fs.writeFile(outAuralSongZipPath, Buffer.from(bytes));
}
