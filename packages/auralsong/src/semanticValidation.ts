...
import { isSupportedAuralSongSchemaVersion, isValidAuralSongSchemaVersion } from "./version";

export interface AuralSongSemanticIssue {
...
): Promise<AuralSongSemanticIssue[]> {
  const issues: AuralSongSemanticIssue[] = [];

  if (!isValidAuralSongSchemaVersion(manifest.schema_version)) {
    issues.push({
      code: "schema_invalid",
