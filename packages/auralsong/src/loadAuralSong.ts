...
export type AuralSongLoadContainerKind = "directory" | "zip";

export type LoadedAuralSong = {
  auralSongPath: string;
  containerKind: AuralSongLoadContainerKind;
...
export async function loadAuralSong(auralSongPath: string): Promise<LoadedAuralSong> {
  const st = await fs.stat(auralSongPath);
  const containerKind: AuralSongLoadContainerKind = st.isDirectory() ? "directory" : "zip";

  if (containerKind === "directory") {
    return loadAuralSongFromDirectory(auralSongPath);
  }
  return loadAuralSongFromZip(auralSongPath);
}

export async function loadAuralSongFromDirectory(auralSongDir: string): Promise<LoadedAuralSong> {
  const manifestAbs = path.join(auralSongDir, "manifest.json");
  ...
  const features: LoadedAuralSong["features"] = {};
  for (const f of FEATURE_FILES) {
    const abs = path.join(auralSongDir, f.relPath);
    if (!(await fileExists(abs))) continue;
    features[f.id] = await readJsonFile(abs);
  }

  const charts: Record<string, unknown> = {};
  const chartsDir = path.join(auralSongDir, "charts");
  try {
    ...
    if (st.isDirectory()) {
      ...
      for (const name of ents) {
        ...
        charts[rel] = await readJsonFile(path.join(auralSongDir, rel));
      }
    }
  } catch {
    // ok
  ...
  const fileList = await listDirFiles(auralSongDir);

  const readBytes = async (relPath: string): Promise<Uint8Array | null> => {
    const rel = normalizeRelPath(relPath);
    const abs = path.join(auralSongDir, rel.split(path.posix.sep).join(path.sep));
  ...
  return {
    ...
    readBytes,
    readJson
  } satisfies LoadedAuralSong;
}

export async function loadAuralSongFromZip(zipAuralSongPath: string): Promise<LoadedAuralSong> {
  const bytes = await fs.readFile(zipAuralSongPath);
  return loadAuralSongFromZipBytes(new Uint8Array(bytes), zipAuralSongPath);
}

export async function loadAuralSongFromZipBytes(zipBytes: Uint8Array, zipAuralSongPath = "<memory>"): Promise<LoadedAuralSong> {
  ...
  return {
    auralSongPath: zipAuralSongPath,
    containerKind: "zip",
    manifest,
    features,
    charts,
