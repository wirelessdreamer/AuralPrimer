...
export interface LoadedAuralSongValidationIssue {
  code: "missing_required_file" | "invalid_json" | "schema_invalid" | "unsupported";
  path: string;
  message: string;
}

export interface LoadedAuralSongValidationResult {
  ok: boolean;
  issues: LoadedAuralSongValidationIssue[];
}
...
export async function validateLoadedAuralSong(pack: LoadedAuralSong): Promise<LoadedAuralSongValidationResult> {
  const issues: LoadedAuralSongValidationIssue[] = [];
