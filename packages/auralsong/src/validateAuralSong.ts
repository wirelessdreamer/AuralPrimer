...
export type AuralSongContainerKind = "directory" | "zip";

export interface AuralSongValidationIssue {
  code:
...
export interface AuralSongValidationResult {
  ok: boolean;
  containerKind: AuralSongContainerKind;
  issues: AuralSongValidationIssue[];
}
...
function validateKnownJson(relPath: string, json: unknown): AuralSongValidationIssue[] {
  const issues: AuralSongValidationIssue[] = [];
...
function validateTimedFeatureOrChart(relPath: string, json: unknown, manifest: AuralSongManifest | null): AuralSongValidationIssue[] {
  if (!manifest) return [];
...
export async function validateAuralSong(auralSongPath: string): Promise<AuralSongValidationResult> {
  const issues: AuralSongValidationIssue[] = [];
  let manifest: AuralSongManifest | null = null;

  // decide container kind
  let containerKind: AuralSongContainerKind;
  try {
    const st = await fs.stat(auralSongPath);
    containerKind = st.isDirectory() ? "directory" : "zip";
  } catch {
    return { ok: false, containerKind: "directory", issues: [{ code: "unreadable", path: auralSongPath, message: "cannot stat path" }] };
  }

  if (containerKind === "directory") {
    // required files
    for (const rel of REQUIRED_TOP_LEVEL) {
      const abs = path.join(auralSongPath, rel);
    ...
    const manifestAbs = path.join(auralSongPath, "manifest.json");
    if (await fileExists(manifestAbs)) {
      try {
        ...
        if (manifestIssues.length === 0) {
          manifest = json as AuralSongManifest;
          issues.push(...(await validateManifestSemantics(manifest, (relPath) => fileExists(path.join(auralSongPath, relPath)))));
        }
      } catch {
    ...
    try {
      const chartsAbs = path.join(auralSongPath, CHARTS_DIR);
  ...
  // zip
  try {
    const bytes = await fs.readFile(auralSongPath);
    ...
    return {
      ok: false,
      containerKind,
      issues: [{ code: "unreadable", path: auralSongPath, message: "unable to read/decompress zip" }]
    };
  }
}
