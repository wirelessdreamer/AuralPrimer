/**
 * Cross-tool regression test: an AuralSong written by aural_ingest must be both
 * discoverable AND pass the TS-side manifest type guard.
 *
 * Context: a Psalm 19 AuralSong imported via `aural_ingest import` did not
 ...
 * Python-side test is at python/ingest/tests/test_auralsong_manifest_schema.py;
 ...
 *  the init_auralsong stage (post-decode/duration update). Kept in sync with
 *  the Python source via the `_emit_init_manifest` helper in
 *  test_auralsong_manifest_schema.py. */
...
describe("aural_ingest auralsong discovery + validation", () => {
  ...
  it("manifest emitted by aural_ingest passes isAuralSongManifest type guard", () => {
    expect(isAuralSongManifest(AURAL_INGEST_MANIFEST)).toBe(true);
  });

  it("discoverAuralSongs finds an aural_ingest-produced directory auralsong", async () => {
    dir = tmpDir();

    const packDir = path.join(dir, "psalm_19_heaven_whispered.auralsong");
    mkdirSync(packDir);
    writeFileSync(
    ...
    expect(found).toEqual([
      {
        name: "psalm_19_heaven_whispered.auralsong",
        kind: "directory",
        path: packDir,
      },
    ]);
  ...
  it("type guard rejects partial manifests missing a required field", () => {
    ...
    for (const field of requiredFields) {
      ...
      delete broken[field];
      expect(isAuralSongManifest(broken)).toBe(false);
    }
  });
  ...
  it("type guard tolerates aural_ingest's empty-string artist for unknown sources", () => {
    ...
    // `args.artist or ""` fallback must not make the auralsong invalid.
    const minimalArtist = { ...AURAL_INGEST_MANIFEST, artist: "" };
    expect(isAuralSongManifest(minimalArtist)).toBe(true);
  });

  it("type guard tolerates aural_ingest's zero-duration init-stage manifest", () => {
    // After init_auralsong runs but before decode_audio updates duration,
    // the auralsong briefly has duration_sec=0.0 on disk. A user who
    // refreshes the library during this window must still see the entry.
    const partial = { ...AURAL_INGEST_MANIFEST, duration_sec: 0.0 };
    expect(isAuralSongManifest(partial)).toBe(true);
  });
  ...
  it("type guard rejects schema_version as int (silent-failure guard)", () => {
    ...
    // schema)'. The TS guard must catch this loudly at the validateAuralSong
