...
function makeLoadedPack(overrides: Partial<LoadedAuralSong> = {}): LoadedAuralSong {
...
describe("validateLoadedAuralSong", () => {
  it("returns ok for valid minimal loaded pack", async () => {
    const res = await validateLoadedAuralSong(makeLoadedPack());
    expect(res.ok).toBe(true);
    expect(res.issues).toHaveLength(0);
  });

  it("reports missing required manifest", async () => {
    const res = await validateLoadedAuralSong(makeLoadedPack({ manifest: null as any }));
  ...
  it("reports schema invalid for malformed feature/chart and invalid json for extras", async () => {
    ...
    const res = await validateLoadedAuralSong(pack);
    expect(res.ok).toBe(false);
