//! SongPack `manifest.json` parsing shared by both Tauri shells.
//!
...
/// One row of the songs-folder scan: a `.songpack` container plus its parsed
...
/// silently hide the songpack from the Play Songs scan with no actionable
/// error.
...
/// Read and summarize `manifest.json` from a directory-style `.songpack`.
pub fn read_dir_manifest(songpack_dir: &Path) -> Result<ManifestSummary, String> {
...
/// Read and summarize `manifest.json` from a zip-style `.songpack`.
pub fn read_zip_manifest(songpack_zip: &Path) -> Result<ManifestSummary, String> {
    let raw = read_zip_manifest_text(songpack_zip)?;
    parse_manifest_json(&raw)
}

/// Read the raw JSON value of `manifest.json` from a directory-style `.songpack`.
pub fn read_dir_manifest_raw(songpack_dir: &Path) -> Result<serde_json::Value, String> {
    let manifest_path = songpack_dir.join("manifest.json");
    let raw = fs::read_to_string(&manifest_path)
        .map_err(|e| format!("read {}: {e}", manifest_path.display()))?;
...
/// Read the raw JSON value of `manifest.json` from a zip-style `.songpack`.
pub fn read_zip_manifest_raw(songpack_zip: &Path) -> Result<serde_json::Value, String> {
    let raw = read_zip_manifest_text(songpack_zip)?;
    parse_manifest_raw(&raw)
}

fn read_zip_manifest_text(songpack_zip: &Path) -> Result<String, String> {
...
mod manifest_scan_tests {
    //! Regression tests for SongPack discovery shared by both shells.
    //!
    ...
    fn read_dir_manifest_raw_finds_manifest_on_disk() {
        ...
        let value = read_dir_manifest_raw(&songpack_dir).expect("raw scan should succeed");
    ...
    fn read_dir_manifest_errors_clearly_when_manifest_missing() {
        ...
        let err =
            read_dir_manifest(&songpack_dir).expect_err("missing manifest must be an error");
    ...
    fn read_dir_manifest_tolerates_utf8_bom() {
        ...
        let summary =
            read_dir_manifest(&songpack_dir).expect("manifest.json with UTF-8 BOM must parse");
        assert_eq!(summary.title.as_deref(), Some("Heaven Whispered"));
    }
}
