//! Shared SongPack contract logic for the AuralPrimer Tauri shells.
//!
//! Both `apps/game/src-tauri` and `apps/desktop/src-tauri` scan the songs
//! folder for `.songpack` containers, read `manifest.json`, and surface a small
...
//!   readers, and the [`ManifestSummary`] / [`SongPackScanEntry`] summary types.
...
pub use manifest::{
    ...
    read_zip_manifest, read_zip_manifest_raw, ManifestSummary, SongPackScanEntry,
};
