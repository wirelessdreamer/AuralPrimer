...
//! window. The frontend re-runs `scan_auralsongs` in response so users see new
//! AuralSongs the moment an external tool (e.g. `aural_ingest import`) drops
//! them in.
//!
//! Design notes:
//!
...
//!   by the game itself (e.g. demo auralsong creation, raw_song imports). The
//!   resulting `scan_auralsongs` refresh is idempotent — the cost is a single
...
/// changed, not what specifically — `scan_auralsongs` on the frontend is the
...
fn spawn_watch(app: AppHandle, songs_folder: &Path) -> Result<ActiveWatch, String> {
    ...
    // songs folder is normally created lazily by `scan_auralsongs`, but on
...
mod tests {
    ...
    fn watcher_observes_file_creation() {
        ...
        let target = dir.path().join("demo.auralsong.placeholder");
    ...
    fn watcher_observes_nested_directory_creation() {
        // Simulates a real `.auralsong/` drop where a directory + manifest
        // appear together.
        let dir = tempdir().expect("tempdir");
        let (sink_tx, sink_rx) = std_channel::<()>();
        let (_watcher, _rx) = spawn_raw_watch(dir.path(), sink_tx);

        let pack = dir.path().join("Example.auralsong");
        std::fs::create_dir(&pack).expect("mkdir auralsong");
