use auralprimer_desktop_tauri::{stem_midi, wav_mix};
use std::fs;
...
#[test]
fn stem_midi_creator_writes_auralsong_artifacts() {
    ...
    let res = stem_midi::create_auralsong(req, &songs_dir).unwrap();
    let out = std::path::PathBuf::from(res.auralsong_path);
    assert!(out.exists());
