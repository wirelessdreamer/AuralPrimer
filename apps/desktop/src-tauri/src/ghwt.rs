...
fn wav_duration_sec(path: &Path) -> Result<f64, String> {
    ...
    while off + 8 <= bytes.len() {
        ...
        if id == b"fmt " {
            if size >= 16 {
                byte_rate = Some(u32::from_le_bytes(bytes[off + 8..off + 12].try_into().unwrap()));
            }
        } else if id == b"data" {
...
fn decode_to_wav(path: &Path, wav_out: &Path, vgmstream_path: Option<String>) -> Result<(), String> {
...
pub fn import_preview_to_songpack_to_folder(
    ...
    let song_ini = ghwt_dlc_root_from_data_root(data_root)
        .join(checksum)
        .join("song.ini");
    let raw_ini = fs::read_to_string(&song_ini)
        .map_err(|e| format!("read {}: {e}", song_ini.display()))?;
...
pub fn import_all_to_folder(
    ...
    for s in songs {
        ...
        match import_preview_to_songpack_to_folder(app, data_root, &s.checksum, songs_folder, vgmstream_path.clone()) {
