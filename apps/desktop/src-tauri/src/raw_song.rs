...
fn midi_bytes_to_notes(midi_bytes: &[u8], track_id: &str) -> Result<Vec<serde_json::Value>, String> {
...
fn find_karaoke_json(root: &Path) -> Option<PathBuf> {
    ...
    matches.sort();
    for p in &matches {
        if p.to_string_lossy().to_ascii_lowercase().contains("newoutput") {
            return Some(p.clone());
        }
    }
...
pub fn import_raw_song_folder(
    ...
    // Derive defaults.
    let folder_name = root.file_name().and_then(|s| s.to_str()).unwrap_or("Raw Song");
    ...
    let preferred = midis
        .iter()
        .find(|p| p.file_name().and_then(|s| s.to_str()).unwrap_or("").contains("Guitar"))
        .cloned()
    ...
    for m in &midis {
        ...
        tracks_out.push(serde_json::json!({"track_id": track_id, "role": role, "name": track_name}));

        let dst = out_dir.join("features").join("midi").join(format!("{track_id}.mid"));
