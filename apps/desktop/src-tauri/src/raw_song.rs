...
mod tests {
    ...
    fn preserve_source_midis_copies_files_and_updates_manifest_reference_paths() {
        ...
        // same base name is the minimum that the scanner pairs with a MIDI.
        let wav = WavPcm16 {
            sample_rate: 48_000,
            channels: 2,
            data: vec![0; 48_000 * 2 * 2],
        ...
        // Second MIDI alongside a bass stem -- exercises the de-dup naming
        // for files that don't collide on base id.
        ...
        // Seed a sidecar-shaped manifest with no reference_paths so we can
        // verify the helper inserts them rather than overwriting siblings.
        ...
        for rel in &added {
            assert!(
                refs_set.contains(rel),
                "manifest references should include {rel}; got {:?}",
                refs_set
            );
        }
        assert_eq!(
