...
fn parse_analysis_overlay(songpack_root: &Path) -> Result<AnalysisImportOverlay, String> {
    let events_json = read_json_file(&songpack_root.join("features").join("events.json"))?;
    ...
    let features_dir = songpack_root.join("features");
...
fn next_temp_songpack_dir(prefix: &str) -> PathBuf {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|dur| dur.as_nanos())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("{prefix}_{nanos}.songpack"))
}
...
fn run_analysis_overlay_import(
    ...
    let temp_out = next_temp_songpack_dir("auralprimer_raw_song_analysis");
    ...
    let req = IngestImportRequest {
        ...
        out_songpack_path: Some(temp_out.to_string_lossy().to_string()),
