use crate::ingest_sidecar::{self, IngestImportRequest, IngestSubcommand};
...
pub struct ImportRawSongFolderRequest {
    ...
    #[serde(default, alias = "chartSource")]
    pub chart_source: RawSongChartSource,

    #[serde(default, alias = "melodicMethod")]
...
mod tests {
    ...
    #[test]
    fn merge_analysis_tracks_replaces_only_melodic_roles() {
        let source_tracks = vec![
            ...
            GameplayTrackNotes {
                track_id: "lead_guitar".to_string(),
        ...
        let analysis_tracks = vec![
            GameplayTrackNotes {
                track_id: "bass_main".to_string(),
                role: "bass".to_string(),
                name: "Bass".to_string(),
                channel: 0,
                notes: vec![TimedMidiNote {
        ...
        let merged = merge_analysis_tracks_into_source(&source_tracks, &analysis_tracks);
        let bass = merged
            .iter()
            .find(|track| track.role == "bass")
            .expect("merged bass track");
        let drums = merged
            .iter()
            .find(|track| track.role == "drums")
            .expect("merged drums track");
        let keys = merged
            .iter()
            .find(|track| track.role == "keys")
            .expect("merged keys track");
        ...
        assert_eq!(
            merged
                ...
                .collect::<Vec<_>>(),
            vec!["drums", "bass", "lead_guitar", "keys"]
        );
        assert_eq!(bass.notes[0].pitch, 43);
        assert_eq!(drums.notes[0].pitch, 36);
        assert_eq!(keys.notes[0].pitch, 72);
    }
    ...
    fn import_raw_song_folder_generates_notes_mid_from_source_midis() {
        ...
        let result = import_raw_song_folder(
            ...
            ImportRawSongFolderRequest {
                ...
                artist: None,
                chart_source: RawSongChartSource::SourceMidi,
                melodic_method: None,
            },
            &songs_root,
        )
...
pub struct ImportRawSongFolderResult {
    ...
    pub chart_source_used: RawSongChartSource,
    pub melodic_method_used: Option<String>,
...
struct AnalysisImportOverlay {
    tracks: Vec<GameplayTrackNotes>,
    beats: Option<serde_json::Value>,
    tempo_map: Option<serde_json::Value>,
    sections: Option<serde_json::Value>,
}

fn is_analysis_overlay_role(role: &str) -> bool {
    matches!(role, "bass" | "rhythm_guitar" | "lead_guitar" | "keys")
}

fn find_stem_part_by_detected_role<'a>(
    parts: &'a [DetectedSongPartScan],
    detected_role: &str,
) -> Option<&'a DetectedSongPartScan> {
    parts
        .iter()
        .find(|part| part.detected_role == detected_role)
}

fn build_analysis_input_stem_paths(scan: &RawSongFolderScan) -> BTreeMap<String, String> {
    let mut paths: BTreeMap<String, String> = BTreeMap::new();
    for role in ["drums", "bass", "keys", "vocals"] {
        if let Some(part) = best_part_for_game_role(&scan.stem_parts, role) {
            paths.insert(role.to_string(), part.path.to_string_lossy().to_string());
        }
    }
    if let Some(part) = find_stem_part_by_detected_role(&scan.stem_parts, "guitar") {
        paths.insert(
            "guitar".to_string(),
            part.path.to_string_lossy().to_string(),
        );
    }
    if let Some(part) = find_stem_part_by_detected_role(&scan.stem_parts, "lead_guitar") {
        paths.insert(
            "lead_guitar".to_string(),
            part.path.to_string_lossy().to_string(),
        );
    }
    if let Some(part) = find_stem_part_by_detected_role(&scan.stem_parts, "rhythm_guitar") {
        paths.insert(
            "rhythm_guitar".to_string(),
            part.path.to_string_lossy().to_string(),
        );
    }
    paths
...
fn parse_analysis_overlay(auralsong_root: &Path) -> Result<AnalysisImportOverlay, String> {
    let events_json = read_json_file(&auralsong_root.join("features").join("events.json"))?;
    let mut track_roles: BTreeMap<String, (String, String)> = BTreeMap::new();
    let mut by_role: BTreeMap<String, GameplayTrackNotes> = BTreeMap::new();

    if let Some(items) = events_json.get("tracks").and_then(|value| value.as_array()) {
        for item in items {
            let Some(track_id) = item.get("track_id").and_then(|value| value.as_str()) else {
                continue;
            };
            let Some(role) = item.get("role").and_then(|value| value.as_str()) else {
                continue;
            };
            if !is_analysis_overlay_role(role) {
                continue;
            }
            let name = item
                .get("name")
                .and_then(|value| value.as_str())
                .unwrap_or(canonical_track_name_for_role(role));
            track_roles.insert(track_id.to_string(), (role.to_string(), name.to_string()));
            by_role
                .entry(role.to_string())
                .or_insert_with(|| GameplayTrackNotes {
    ...
    if let Some(items) = events_json.get("notes").and_then(|value| value.as_array()) {
        for item in items {
            let Some(track_id) = item.get("track_id").and_then(|value| value.as_str()) else {
                continue;
            };
            let Some((role, _name)) = track_roles.get(track_id) else {
                continue;
            };
            let Some(t_on) = item.get("t_on").and_then(|value| value.as_f64()) else {
                continue;
            };
            let Some(t_off) = item.get("t_off").and_then(|value| value.as_f64()) else {
                continue;
            };
            let Some(pitch) = item
                .get("pitch")
            ...
            let velocity = item
                ...
                .unwrap_or(90.0);
            if let Some(track) = by_role.get_mut(role) {
                track.notes.push(TimedMidiNote {
                    t_on,
                    t_off: t_off.max(t_on + 0.01),
                    pitch,
                    velocity: if velocity > 1.0 {
    ...
    let features_dir = auralsong_root.join("features");
    Ok(AnalysisImportOverlay {
        tracks,
        beats: read_json_file(&features_dir.join("beats.json")).ok(),
        tempo_map: read_json_file(&features_dir.join("tempo_map.json")).ok(),
        sections: read_json_file(&features_dir.join("sections.json")).ok(),
    })
}

fn merge_analysis_tracks_into_source(
    source_tracks: &[GameplayTrackNotes],
    analysis_tracks: &[GameplayTrackNotes],
) -> Vec<GameplayTrackNotes> {
    let mut merged: BTreeMap<String, GameplayTrackNotes> = BTreeMap::new();
    for track in source_tracks {
        merged.insert(track.role.clone(), track.clone());
    }
    for track in analysis_tracks {
        if is_analysis_overlay_role(&track.role) && !track.notes.is_empty() {
            merged.insert(track.role.clone(), track.clone());
        }
    }

    let mut tracks: Vec<GameplayTrackNotes> = merged.into_values().collect();
    for track in &mut tracks {
        track.notes.sort_by(|a, b| {
            a.t_on
                .partial_cmp(&b.t_on)
...
fn analysis_variant_title(title: &str, method: &str) -> String {
    format!("{title} (analysis: {method})")
}

fn next_temp_auralsong_dir(prefix: &str) -> PathBuf {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|dur| dur.as_nanos())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("{prefix}_{nanos}.auralsong"))
}

fn run_analysis_overlay_import(
    app: &AppHandle,
    scan: &RawSongFolderScan,
    source_folder: &Path,
    title: &str,
    artist: &str,
    melodic_method: &str,
) -> Result<AnalysisImportOverlay, String> {
    let temp_out = next_temp_auralsong_dir("auralprimer_raw_song_analysis");
    let input_stem_paths = build_analysis_input_stem_paths(scan);
    let config = if input_stem_paths.is_empty() {
        serde_json::json!({})
    } else {
        serde_json::json!({
            "disable_stem_separation": true,
            "input_stem_paths": input_stem_paths,
        })
    };

    let req = IngestImportRequest {
        source_path: source_folder.to_string_lossy().to_string(),
        out_auralsong_path: Some(temp_out.to_string_lossy().to_string()),
        subcommand: Some(IngestSubcommand::ImportDir),
        profile: Some("full".to_string()),
        config: Some(config.to_string()),
        title: Some(analysis_variant_title(title, melodic_method)),
        artist: Some(artist.to_string()),
        duration_sec: None,
        drum_filter: Some("combined_filter".to_string()),
        melodic_method: Some(melodic_method.to_string()),
        shifts: Some(1),
        multi_filter: Some(false),
        ingest_binary_path: None,
    };

    let result = ingest_sidecar::run_ingest_import_with_progress(req, Some(app))?;
    if !result.ok {
        let reason = if !result.stderr.trim().is_empty() {
            ...
            "analysis sidecar failed without output".to_string()
        };
        fs::remove_dir_all(&temp_out).ok();
        return Err(format!(
            "analysis import failed for '{}' with method '{}': {}",
    ...
    let overlay = parse_analysis_overlay(&temp_out)?;
    fs::remove_dir_all(&temp_out).ok();
    Ok(overlay)
}
...
pub fn import_raw_song_folder(
    ...
    let manifest_title = if let Some(method) = melodic_method_used.as_deref() {
        analysis_variant_title(&title, method)
    } else {
        title.clone()
    };

    // Build output folder name.
    let base = format!("{}_{}", sanitize_id(&artist), sanitize_id(&manifest_title));
    ...
    let mut normalized_source_tracks: Vec<GameplayTrackNotes> = vec![];
    ...
    } else if let Some(_timing) = canonical_timing.as_ref() {
        for track in &source_tracks {
            ...
            normalized_source_tracks.push(normalized_track);
        }
        if normalized_source_tracks.is_empty() {
            warnings.push(
    ...
    let analysis_overlay = if let Some(method) = melodic_method_used.as_deref() {
        let app = app.ok_or_else(|| {
            "analysis melodic Suno import requires a Tauri AppHandle for sidecar execution"
                .to_string()
        })?;
        warnings.push(format!(
            "Running melodic analysis overlay with method '{}'.",
            method
        ));
        Some(run_analysis_overlay_import(
            app, &scan, &root, &title, &artist, method,
        )?)
    } else {
        None
    };

    let analysis_roles_available: BTreeSet<String> = analysis_overlay
        .as_ref()
        .map(|overlay| {
            overlay
                .tracks
                ...
                .map(|track| track.role.clone())
                .collect()
        })
        .unwrap_or_default();

    let mut final_tracks = match chart_source_used {
        RawSongChartSource::SourceMidi => normalized_source_tracks.clone(),
        RawSongChartSource::AnalysisMelodic => {
            if let Some(overlay) = analysis_overlay.as_ref() {
                if normalized_source_tracks.is_empty() {
                    overlay.tracks.clone()
                } else {
                    merge_analysis_tracks_into_source(&normalized_source_tracks, &overlay.tracks)
                }
            } else {
                normalized_source_tracks.clone()
            }
        }
    };
    final_tracks.retain(|track| !track.notes.is_empty());

    let analysis_roles_used: BTreeSet<String> = final_tracks
        .iter()
        .filter(|track| analysis_roles_available.contains(&track.role))
        .map(|track| track.role.clone())
        .collect();

    let note_timing = canonical_timing
        .clone()
        .unwrap_or_else(default_midi_timing_map);
    let (beats, tempo, sections) = if let Some(timing) = canonical_timing.as_ref() {
        ...
        let sections = generate_sections_from_beats(duration_sec, &beats, 8);
        (beats, tempo, sections)
    } else if let Some(overlay) = analysis_overlay.as_ref() {
        match (
            overlay.beats.clone(),
            overlay.tempo_map.clone(),
            overlay.sections.clone(),
        ) {
            (Some(beats), Some(tempo), Some(sections)) => (beats, tempo, sections),
            _ => {
                warnings.push(
                    "Analysis import did not emit beat/tempo metadata; using a neutral fallback grid."
                        .to_string(),
                );
                build_neutral_grid()
            }
    ...
    if !final_tracks.is_empty() {
        ...
        for track in &final_tracks {
            let track_source = if analysis_roles_used.contains(&track.role) {
                "analysis_melodic_import"
            } else {
        ...
        if chart_source_used == RawSongChartSource::AnalysisMelodic
            && !analysis_roles_used.is_empty()
        {
            if canonical_timing.is_some() {
                midi_timing_trust = "normalized_source";
                chart_timing_status = "normalized_source_with_analysis_melodic_overlay";
                timing_authority = "normalized_source";
            } else {
                midi_timing_trust = "analysis_audio";
                chart_timing_status = "analysis_melodic_audio_overlay";
                timing_authority = "analysis_audio";
            }
            if let Some(method) = melodic_method_used.as_deref() {
                let roles = analysis_roles_used.iter().cloned().collect::<Vec<_>>();
                warnings.push(format!(
                    "Replaced melodic source MIDI tracks with analysis-derived notes for {} using '{}'.",
                    roles.join(", "),
                    method,
                ));
            }
        } else if canonical_timing.is_some() && !normalized_source_tracks.is_empty() {
            midi_timing_trust = "normalized_source";
            chart_timing_status = "normalized_from_source_midi";
            timing_authority = "normalized_source";
        } else {
            midi_timing_trust = "fallback_grid";
            chart_timing_status = "fallback_grid";
            timing_authority = "fallback_grid";
        }
    } else if chart_source_used == RawSongChartSource::AnalysisMelodic {
        warnings.push(
            "Analysis import did not produce any melodic tracks; copied source assets without a gameplay chart."
                .to_string(),
        );
    }

    if canonical_timing.is_some() && !normalized_source_tracks.is_empty() {
    ...
    let manifest = serde_json::json!({
        ...
        "source": {
            ...
            "chart_source": chart_source_used,
            "melodic_method": melodic_method_used.clone(),
