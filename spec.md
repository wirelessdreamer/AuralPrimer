...
To accelerate adoption without constraining internal capabilities, AuralPrimer must support **pluggable importers** that convert external sources into SongPacks (e.g., audio-only, user MIDI, proprietary_archive_import DE, and other ecosystems).
...
- The desktop app must provide a **Configure → Import proprietary_archive_import songs** section.
